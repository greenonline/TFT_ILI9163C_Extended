...
//constructors
	...
	TFT_ILI9163C_Extended::TFT_ILI9163C_Extended(uint8_t cspin,uint8_t dcpin,uint8_t rstpin,uint8_t mosi,uint8_t sclk) : TFT_ILI9163C(_TFTWIDTH,_TFTHEIGHT)
	{
		_cs   = cspin;
		_rs   = dcpin;
		_rst  = rstpin;
...
#else
	TFT_ILI9163C_Extended::TFT_ILI9163C_Extended(uint8_t cspin,uint8_t dcpin,uint8_t rstpin) : TFT_ILI9163C(_TFTWIDTH,_TFTHEIGHT)
	{
		_cs   = cspin;
		_rs   = dcpin;
		_rst  = rstpin;
...
//  void TFT_ILI9163C_Extended::setCursor(int16_t x, int16_t y) {
//      TFT_ILI9163C::setCursor(x*kCharacterWidth*kCharacterSize, y*kCharacterHeight*kCharacterSize);
//  }
// Delete - used only in Extended, not Extended_Char

  void TFT_ILI9163C_Extended::setCursorChar(int16_t x, int16_t y) {
      TFT_ILI9163C::setCursor(x*kCharacterWidth*kCharacterSize, y*kCharacterHeight*kCharacterSize);
  }
